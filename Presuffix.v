...
Require Import Lia.
Require Import ListLib.Inductive.
...
Proof.
  ...
  + pose proof prefix_length _ _ H.
    ...
    reflexivity.
  + destruct H.
    exists (to_sublist_pos (length l1) (length l2) l2).
...
Proof.
  ...
  + pose proof suffix_length _ _ H.
    ...
    f_equal. lia.
  + destruct H.
    exists (to_sublist_pos 0 (length l2 - length l1) l2).
    eapply (nth_ext _ _ default default).
    - rewrite length_app.
      rewrite length_sublist by lia.
      lia.  
...
Proof.
  intros.
  split; intros.
  + destruct (snoc_destruct res) as [? | [ch0 [res' ?] ] ]; subst res.
    - left.
      tauto.
    - right.
      exists res'.
...
Lemma prefix_sublist_iff {A: Type}:
  forall (l0 l: list A) i,
    0 <= i <= length l ->
    l0 is_a_prefix_of (to_sublist_pos 0 i l) <->
...
Lemma suffix_sublist_cons_iff {A: Type}:
  forall (l0 l: list A) i,
    1 <= i <= length l ->
    l0 is_a_suffix_of (to_sublist_pos 1 i l) <->
    length l0 <= i - 1 /\ l0 is_a_suffix_of (to_sublist_pos 0 i l).
Proof.
  intros.
